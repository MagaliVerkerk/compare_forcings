...
species, properties = read_properties('./data/species_configs_properties_cmip6.csv')
f.define_species(species, properties)
f.allocate()

f.fill_species_configs('./data/species_configs_properties_cmip6.csv')

df_emissions_init = pd.read_csv("./data/baseline_emissions_CMIP6_1850.csv",index_col=0,)
df_concentration_init = pd.read_csv("./data/baseline_concentration_CMIP6_1850.csv",index_col=0,)
...
df_volcanic = pd.read_csv('./data/cmip6_forcings/volcanic_forcing_monthly_cmip6.csv', index_col=0)
# overwrite volcanic
...
df_solar = pd.read_csv('./data/cmip6_forcings/solar_forcing_cmip6.csv', index_col=0)
fill(f.forcing, df_solar['cmip6'].to_numpy()[:, None, None], specie='Solar')

df_landuse = pd.read_csv('./data/cmip6_forcings/land_use_forcing_LUH2.csv', index_col=0)
fill(f.forcing, df_landuse.loc[start_date:end_date,'LUH2-CMIP6'].to_numpy()[:, None, None], specie='Land use')
...
plt.plot(f.timebounds, np.mean(np.array(f.temperature.loc[dict(scenario='ssp245', layer=0)]), axis=1), color='k', lw=2.5)
plt.xlabel('time, years')
plt.ylabel('temperature')
plt.show()


ghg_forcing = np.zeros((165, 43))
aerosol_forcing = np.column_stack((np.array(f.forcing.loc[dict(scenario='ssp245', config='4520', specie='Aerosol-radiation interactions')]), 
                                   np.array(f.forcing.loc[dict(scenario='ssp245', config='4520', specie='Aerosol-cloud interactions')])))

other_anthro_forcing = np.column_stack((np.array(f.forcing.loc[dict(scenario='ssp245', config='4520', specie='Ozone')]), 
                                        np.array(f.forcing.loc[dict(scenario='ssp245', config='4520', specie='Land use')])))

natural_forcing = np.column_stack((np.array(f.forcing.loc[dict(scenario='ssp245', config='4520', specie='Solar')]), 
                                   np.array(f.forcing.loc[dict(scenario='ssp245', config='4520', specie='Volcanic')])))
...
for specie in species:
    if properties[specie]['input_mode'] == 'concentration':
        ...
        plt.plot(f.timebounds, f.forcing.loc[dict(scenario='ssp245', config='4520', specie=specie)], color='goldenrod')
